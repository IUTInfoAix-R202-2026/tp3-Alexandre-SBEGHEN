...
public class SiteCarte extends HBox {
  ...
  private void majBadge(int jours) {
    ...
    labelBadge.setText(String.format("Il y a %dj", jours));
    if (jours < 0) {
      labelBadge.setText("Jamais utilisé");
      labelBadge.getStyleClass().addAll("badge-cold");
    } else if (jours < 7) {
      labelBadge.getStyleClass().addAll("badge-fresh");
    } else if (jours <= 30) {
      labelBadge.getStyleClass().addAll("badge-stale");
    } else {
      labelBadge.getStyleClass().addAll("badge-cold");
    }
  }
