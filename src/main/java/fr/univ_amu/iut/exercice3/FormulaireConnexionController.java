...
public class FormulaireConnexionController {
        ...
        new BooleanBinding() {
          ...
          public boolean computeValue() {
            ...
            for (char i : pwd.toCharArray()) {
              if (i >= 'A' && i <= 'Z') contientMaj = true;
              if (i >= '0' && i <= '9') contientChiffre = true;
            }

            return pwd.length() < 8 || !contientMaj || !contientChiffre;
          }
        };
  ...
  private void valider() {
    ...
    labelMessage.setText(
        String.format(
            "%s %s", champIdentifiant.getText(), "*".repeat(champMotDePasse.getLength())));
  }
